...
namespace Hd.Web.Extensions
{
    ...
    public class GridController : WebControl, IPostBackEventHandler
    {
        ...
        public GridController(GridView grid, Pager pager, BusinessQuery businessQuery)
            : this(grid, pager, businessQuery, null, null)
        {
        }
        ...
        public GridController(GridView grid, Pager pager,
                              BusinessQuery businessQuery, Type entityBaseType, string filters)
        {
            IsRequiredToBindGrid = false;
            if (grid == null)
        ...
        #endregion WebControl Event Handlers
        ...
        public void BindGrid(bool bindFlag)
        {
            ...
            {
                var columns = _filters.Split(',');
                foreach (string column in columns)
                {
                    if (HttpContext.Current.Request.QueryString[column] == null) continue;
                    var searchFilter = new SearchFilter { SearchPattern = HttpContext.Current.Request.QueryString[column] };
                    searchFilter.AppendSimpleSearchPattern(_businessQuery.Query, column, CompareOperator.Equal);
                }
            }
